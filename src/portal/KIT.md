...
| `ThemeDisc` | `size?=36, glyph?=18` | `.rr-pt-theme` | The `SunMoon` switch (src/ui/ThemeSwitch.tsx) in its portal cut: sun in light, moon in dark; flips through `useTheme().toggleFrom` — the circular reveal from the disc itself, as /login's and the reader's; pins `rr-theme`. |
