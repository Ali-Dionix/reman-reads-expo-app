...
| `ThemeDisc` | `size?=36, glyph?=18` | `.rr-pt-theme` | The `SunMoon` switch (src/ui/ThemeSwitch.tsx) in its portal cut: sun in light, moon in dark; flips through the nearest `ThemeStage`'s `reveal` (src/theme/ThemeStage.tsx) — the circular reveal from the disc itself, the screen held still on both sides of the edge, as /login's and the reader's; pins `rr-theme`. |
...
| `GateSheet` (src/portal) | `open, onClose, onContinue, pitch?="imports"|"voices"`; `gateNext(key)` | `.rr-im.is-upgrade` (importUpgrade.ts) | The locked panel, full-screen in place; Continue goes out through `openSignedIn` — Home's cells with `gateNext(key)`, the site's `/account?add=<key>`; the narrator sheet's padlocks (every voice but the one playing, since 15 Sep 2026) with `pitch="voices"`, the site's Settings — signed in by the handoff (a plain `/login?next=` when the handoff cannot be had). Opened only for a reader the entitlement has not opened; a subscriber's tap goes straight through. |
